...
async def run_test(n=5):
    session = _get_or_create_api_session('loadtest','cairene','female')
    pipeline = _make_voice_pipeline_for_session(session)
    for i in range(n):
        print(f'=== Run {i+1} ===')
        before_vram = tts_engine._get_vram_usage_percent() if hasattr(tts_engine,'_get_vram_usage_percent') else 0.0
        start = time.perf_counter()
        out = await pipeline.run_audio_turn(audio_bytes=audio, mime_type='audio/wav', tts_dialect='cairene', tts_gender='female', language='ar', include_tts=True)
        duration = (time.perf_counter() - start)*1000
        after_vram = tts_engine._get_vram_usage_percent() if hasattr(tts_engine,'_get_vram_usage_percent') else 0.0
        ...
        print('TTS format, audio bytes:', out.get('tts',{}).get('audio_format'), len(out.get('tts',{}).get('audio_bytes',b'')))
        print('VRAM before=%.1f%% after=%.1f%%' % (before_vram, after_vram))
        await asyncio.sleep(0.6)
    return

if __name__ == '__main__':
    asyncio.run(run_test(5))
